...
class FakeOdooClient:
    ...
    def search_read(self, model, domain, fields, limit=None):
        ...
        if model == "sale.order.line": return [{"id": 90, "product_id": [15, "Perfume 100ML"], "product_uom_qty": 10, "price_unit": 12.3}]
        if model == "product.product": return [{"id": 15, "display_name": "Perfume 100ML", "default_code": "PERF100"}]
        return []
...
def test_sprint4_full_approval_gate_governance_flow(monkeypatch):
    ...
    req_response = client.post(f"/v1/product/skills/{skill_id}/approval-request", json={
    ...
    decision_response = client.post(f"/v1/product/skills/{skill_id}/approval-decision", json={
        "decided_by": _APPROVER, "decision": "approve", "reason": "Guards verified. Dry-run proof passed.",
    })
    ...
    assert "dry-run" in summary["next_required_action"].lower() or "gate" in summary["next_required_action"].lower() or "approved" in summary["next_required_action"].lower()
...
def test_sprint4_reject_decision(monkeypatch):
    ...
    client.post(f"/v1/product/skills/{skill_id}/approval-request", json={
        "requested_by": _OPERATOR, "reason": "Review this.", "context": {},
    })
    decision_response = client.post(f"/v1/product/skills/{skill_id}/approval-decision", json={
        "decided_by": _APPROVER, "decision": "reject", "reason": "Does not meet criteria.",
    })
    assert decision_response.status_code == 200
...
def test_sprint4_invalid_decision_returns_422(monkeypatch):
    ...
    skill_id = _make_compiled_skill(client, monkeypatch)
    client.post(f"/v1/product/skills/{skill_id}/approval-request", json={
        "requested_by": _OPERATOR, "reason": "Review.", "context": {},
    })

    response = client.post(f"/v1/product/skills/{skill_id}/approval-decision", json={
        "decided_by": _APPROVER, "decision": "execute_now", "reason": "bad decision",
    })
