...
def test_connection_context_resolves_for_approved_skill(monkeypatch):
    ...
    response = client.get(f"/v1/product/skills/{skill_id}/connection-context")
...
def test_connection_context_missing_for_unknown_skill():
    client = TestClient(app)
    response = client.get("/v1/product/skills/ghost_skill/connection-context")
...
def test_live_read_policy_blocked_for_unknown_skill():
    client = TestClient(app)
    response = client.get("/v1/product/skills/unknown_skill/live-read-policy")
    assert response.status_code == 200
    body = response.json()
    assert body["passed"] is False
    assert body["can_execute_real_writes"] is False
    assert body["allow_real_odoo_reads"] is True
    assert len(body["violations"]) > 0


def test_live_read_policy_passes_for_approved_skill(monkeypatch):
    client = TestClient(app)
    skill_id = _make_approved_skill(client, monkeypatch)

    response = client.get(f"/v1/product/skills/{skill_id}/live-read-policy")
    assert response.status_code == 200
    body = response.json()
    assert body["can_execute_real_writes"] is False
...
def test_list_live_read_requests_for_skill(monkeypatch):
    client = TestClient(app)
    skill_id = _make_approved_skill(client, monkeypatch)
...
def test_run_live_read_executes_real_reads(monkeypatch):
    ...
    assert body["run_id"].startswith("lr_run_")
    assert body["status"] == "completed"
    assert body["can_execute_real_writes"] is False
    assert body["real_erp_writes_enabled"] is False
    assert body["allow_real_odoo_reads"] is True
    assert body["real_read_count"] >= 1
...
def test_live_read_produces_evidence_records(monkeypatch):
    ...
    evidence_response = client.get(f"/v1/product/live-read-runs/{run_id}/live-evidence")
    ...
    for ev in evidence:
        ...
        assert ev["live_read_run_id"] == run_id
...
def test_get_live_read_run_by_id(monkeypatch):
    ...
    get_response = client.get(f"/v1/product/live-read-runs/{run_id}")
...
def test_live_read_request_never_allows_real_writes(monkeypatch):
    client = TestClient(app)
    skill_id = _make_approved_skill(client, monkeypatch)
