...
from erpguard.db.repositories import create_connection, list_approval_decisions_for_skill
...
def _make_request(monkeypatch):
    ...
    ApprovalRequestService(session).create(skill_id=compiled.skill_id, requested_by=_OPERATOR, reason="Governance review.")
...
def test_approval_decision_history_lists_all_decisions(monkeypatch):
    ...
    ApprovalDecisionService(session).decide(skill_id=skill_id, decided_by=_APPROVER, decision="approve", reason="ok")
