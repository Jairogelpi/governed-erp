...
class FakeOdooClient:
    ...
    def search_read(self, model, domain, fields, limit=None):
        ...
        if model == "product.product":
            return [{"id": 15, "display_name": "P 100ML", "default_code": "P100"}]
        return []
...
def test_create_execution_request_for_approved_skill(monkeypatch):
    ...
    response = client.post(
        f"/v1/product/skills/{skill_id}/execution-requests",
        json={
            "requested_by": _OPERATOR,
            "inputs": {"dry_run": True},
        },
...
def test_execution_timeline_returns_steps(monkeypatch):
    ...
    timeline_response = client.get(
        f"/v1/product/skills/{skill_id}/execution-runs/{run_id}/timeline"
    )
...
def test_blocked_writes_list_is_returned(monkeypatch):
    ...
    blocked_response = client.get(
        f"/v1/product/skills/{skill_id}/execution-runs/{run_id}/blocked-writes"
    )
...
def test_execution_request_rejected_for_unknown_skill():
    client = TestClient(app)
    response = client.post(
        "/v1/product/skills/ghost_skill/execution-requests",
        json={
            "requested_by": _OPERATOR,
            "inputs": {},
        },
