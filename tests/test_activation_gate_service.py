...
def test_activation_gate_open_after_full_flow(monkeypatch):
    ...
    ApprovalRequestService(session).create(
        skill_id=skill_id, requested_by=_OPERATOR, reason="Governance review."
    )
    ApprovalDecisionService(session).decide(
        skill_id=skill_id, decided_by=_APPROVER, decision="approve", reason="All good."
    )
    ...
    real_exec_check = next(
        (
            ...
            if (c.check_id if hasattr(c, "check_id") else c["check_id"]) == "real_execution_blocked"
        ),
        None,
    )
    assert real_exec_check is not None
    passed = (
        real_exec_check.passed if hasattr(real_exec_check, "passed") else real_exec_check["passed"]
    )
