...
Relocated from `README.md`'s top section by Spec 95 (Phase 22) Sec 38 --
the README now leads with the Definition of Done instead of this
chronological engineering narrative. This is **not** legacy/historical
material (that's `docs/legacy/releases.md`, pre-Phase-0 sprint era) --
everything below describes the current, live system, just at a level of
detail too long for a README's top section.
...
- C8.1 read-only ORM/schema audit of `erpguard/db/models.py` (see
  [docs/architecture/c8_1_orm_schema_audit.md](c8_1_orm_schema_audit.md))
...
path (tables recreated, zero rows) — all against SQLite. Verified in CI
against PostgreSQL and via `docker build` on every push.
...
in
[`docs/demo/phase17_governed_confirmation_live_staging_evidence.json`](../demo/phase17_governed_confirmation_live_staging_evidence.json).
...
duplicate). See
[docs/demo/backend_rc_live_pricing_scenario_evidence.json](../demo/backend_rc_live_pricing_scenario_evidence.json).
A generated (fixture-backed, not live-Odoo) full lifecycle evidence bundle
is in
[docs/demo/backend_rc_decision_to_outcome_evidence.json](../demo/backend_rc_decision_to_outcome_evidence.json).
Full detail: [docs/specs/92_governed_decision_to_outcome_backend_rc.md](../specs/92_governed_decision_to_outcome_backend_rc.md).

Phase 20 (ERPRiskBench, Spec 93) and Phase 21 (Product Web Application,
Spec 94) narratives live in their own specs
([93](../specs/93_phase20_erpriskbench_and_experiments.md),
[94](../specs/94_phase21_product_web_application.md)) and in
`CHANGELOG.md` rather than being restated here.

The exact phase gates and no-goals are defined in the [master implementation
specification](../specs/84_erpguard_evolution_master_spec.md).
